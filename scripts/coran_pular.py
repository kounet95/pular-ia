...
FICHIER_TELEGRAM_BASE = PROJET_ROOT / "corpus-pular" / "processed" / "telegram" / "base_connaissance.json"
...
def _mots_recherche(texte: str) -> set[str]:
    texte = unicodedata.normalize("NFKD", texte.lower())
    texte = "".join(c for c in texte if not unicodedata.combining(c))
    return set(re.findall(r"[\w']+", texte, flags=re.UNICODE))


def rechercher_transcriptions(q: str, n: int = 8) -> list[dict]:
    """Recherche les passages pertinents dans les audios déjà transcrits."""
    mots_requete = _mots_recherche(q)
    if not mots_requete:
        return []

    candidats = []
    fichiers = list(DOSSIER_TRANSCRIPTIONS.glob("*.json")) if DOSSIER_TRANSCRIPTIONS.exists() else []
    for fichier in fichiers:
        ...
        trouves_dans_fichier = False
        for segment in data.get("segments", []):
            if segment.get("sans_voix"):
                continue
            texte = (segment.get("texte") or "").strip()
            if not texte:
                continue
            mots_texte = _mots_recherche(texte)
            score = len(mots_requete & mots_texte) / len(mots_requete)
            if score > 0:
                trouves_dans_fichier = True
                candidats.append((score, {
                    "source": "telegram_audio",
                    "fichier": data.get("fichier", str(fichier)),
                    "nom": data.get("nom", fichier.name),
                    "debut": segment.get("debut"),
                    "fin": segment.get("fin"),
                    "texte": texte,
                    "langue": data.get("langue_detect", "?"),
                    "score": round(score, 3),
                }))

        # Certains anciens résultats Whisper marquent à tort tous les segments
        # comme sans voix. La transcription complète reste alors recherchable.
        if not trouves_dans_fichier:
            texte = (data.get("texte") or "").strip()
            mots_texte = _mots_recherche(texte)
            score = len(mots_requete & mots_texte) / len(mots_requete) if texte else 0
            if score > 0:
                candidats.append((score, {
                    "source": "telegram_audio",
                    "fichier": data.get("fichier", str(fichier)),
    ...
    # Les transcriptions Telegram fusionnées peuvent contenir un texte complet
    # sans fichier JSON séparé dans processed/transcriptions.
    if FICHIER_TELEGRAM_BASE.exists():
        try:
            with open(FICHIER_TELEGRAM_BASE, encoding="utf-8") as f:
                base = json.load(f)
        except (OSError, json.JSONDecodeError):
            base = []
        for message in base:
            texte = (message.get("transcription") or "").strip()
            if not texte:
                continue
            mots_texte = _mots_recherche(texte)
            score = len(mots_requete & mots_texte) / len(mots_requete)
            if score > 0:
                candidats.append((score, {
                    "source": "telegram_audio",
                    "canal": message.get("canal", "?"),
                    "message_id": message.get("message_id"),
                    "fichier": message.get("fichier_local", ""),
                    "texte": texte,
                    "langue": message.get("langue_detect", "?"),
                    "score": round(score, 3),
                }))

    candidats.sort(key=lambda item: -item[0])
    return [item for _, item in candidats[:n]]
